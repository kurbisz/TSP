...
def draw():
    ...
    plt.savefig('longTermList.png', bbox_inches='tight')
    plt.clf()


    file = open('longTermListKick.csv')
    file.readline()
    names = ['' for n in range(10)]
    longTermList = [[0 for n in range(10)] for j in range(8)]
    for i in range(10):
    ...
    r1 = np.arange(10)*2
    plt.title('Porównanie taboo search dla listy długoterminowej wykonującej Kick co n kroków')
    color = ['black', 'red', 'darkorange', 'greenyellow', 'green', 'blue']
    plt.bar(r1 - 1.5 * l, longTermList[0], color=color[0], width=l, label='Bez listy długoterminowej')
    plt.bar(r1 - 0.5 * l, longTermList[1], color=color[1], width=l, label='n = 50')
    plt.bar(r1 + 0.5 * l, longTermList[2], color=color[2], width=l, label='n = 100')
    ...
    plt.yscale('log')
    plt.yticks([1000, 10000, 100000])
    plt.savefig('longTermListKick.png', bbox_inches='tight')
    plt.clf()
